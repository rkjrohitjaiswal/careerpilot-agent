...
export function isCareerPlan(value: unknown): value is CareerPlan {
  ...
  if (!["profileStrength", "topMatch", "roadmapProgress"].every((key) => isNumber(value[key]))) return false;
  ...
  const validSkills = value.skills.every((skill) => isRecord(skill) && isString(skill.name) && isNumber(skill.current) && isNumber(skill.target) && isString(skill.priority) && isString(skill.recommendation) && isTone(skill.tone));
  const validPaths = value.paths.every((path) => isRecord(path) && isString(path.title) && isNumber(path.match) && isString(path.salary) && isString(path.explanation) && isStringArray(path.strengths) && isStringArray(path.missingSkills) && isString(path.nextStep) && isTone(path.color));
  ...
  const validResume = isNumber(resume.score) && isStringArray(resume.strengths) && isStringArray(resume.weaknesses) && isStringArray(resume.keywords) && Array.isArray(resume.feedback) && resume.feedback.every((item) => isRecord(item) && isString(item.label) && isString(item.title) && isString(item.detail));
...
async function openAICareerProvider(profile: Profile): Promise<CareerPlan> {
  ...
  if (!isCareerPlan(parsed)) throw new Error("career-invalid");
  return { ...parsed, source: "provider", generatedAt: "just now" };
}
...
async function openAIResumeProvider(input: { resumeText: string; targetCareer: string; profile?: Partial<Profile> }): Promise<ResumeAnalysis> {
  ...
  if (!isResumeAnalysis(parsed)) throw new Error("resume-invalid");
